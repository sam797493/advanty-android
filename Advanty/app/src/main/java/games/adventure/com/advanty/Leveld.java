...
import games.adventure.com.advanty.ObstacleBonusd;
import games.adventure.com.advanty.ObstacleJumpd;
import games.adventure.com.advanty.OpenGLRenderer;
import games.adventure.com.advanty.Playerd;
...
public class Leveld {
	...
	public Leveld(Context context, OpenGLRenderer glrenderer, int _width, int _heigth) {
		if(Settings.RHDEBUG)
		...
		obstacleDataJumper = new ObstacleJumpd[maxObstaclesJumper];
		leftJumperIndex = 0;
		rightJumperIndex = maxObstaclesJumper;
		
		obstacleDataBonus = new ObstacleBonusd[maxObstaclesBonus];
		leftBonusIndex = 0;
		rightBonusIndex = maxObstaclesBonus;

		obstacleSlowImg = Util.loadBitmapFromAssets("game_obstacle_slow.png");
		obstacleBonusImg =Util.loadBitmapFromAssets("game_obstacle_bonus4.png");
		
		Block.setTextureLeft(Util.loadBitmapFromAssets("game_block_left4.png"));
		Block.setTextureMiddle(Util.loadBitmapFromAssets("game_block_middle4.png"));
		Block.setTextureRight(Util.loadBitmapFromAssets("game_block_right4.png"));
	...
	private void appendBlockToEnd(int BlockNumber)
	{
		...
		if(BlockNumber==-1){
			...
			if(distance <= Playerd.width)
				distance = Playerd.width+10;
		}else{	
			distance = Playerd.width+5;
			switch (BlockNumber){
				case 1:
					newHeight=oldHeight; 
...
//		newLeft = lastBlock.BlockRect.right + distance; 
	...
	private void initializeObstacles(Boolean firstTime)
	{
		for(int i = 0; i < maxObstaclesJumper; i++)
		{
			if (firstTime)
			{
				obstacleDataJumper[i] = new ObstacleJumpd(-1000, 0, 0.9f, obstacleJumperWidth, obstacleJumperHeight, 'j', 60, 4);
			}
		...
		for(int i = 0; i < maxObstaclesBonus; i++)
		{
			if (firstTime)
			{
				obstacleDataBonus[i] = new ObstacleBonusd(-1000, 0, 0.9f, obstacleBonusWidth, obstacleBonusHeight, 'b');
	...
	private void appendObstaclesToEnd(Boolean spawnJumper, Boolean spawnSlower, Boolean spawnBonus)
	{
		...
		if (spawnJumper)
		{
			...
			float obstacleLeft;
			ObstacleJumpd newJumpObstacle = obstacleDataJumper[leftJumperIndex];
		...
		if (spawnBonus)
		{
			...
			ObstacleBonusd newBonus = obstacleDataBonus[leftBonusIndex];
