...
import games.adventure.com.advanty.ObstacleBonusb;
import games.adventure.com.advanty.ObstacleJumpb;
import games.adventure.com.advanty.OpenGLRenderer;
import games.adventure.com.advanty.Playerb;
...
public class Levelb {
	...
	public Levelb(Context context, OpenGLRenderer glrenderer, int _width, int _heigth) {
		if(Settings.RHDEBUG)
		...
		obstacleDataJumper = new ObstacleJumpb[maxObstaclesJumper];
		leftJumperIndex = 0;
		rightJumperIndex = maxObstaclesJumper;

		obstacleDataBonus = new ObstacleBonusb[maxObstaclesBonus];
		leftBonusIndex = 0;
		rightBonusIndex = maxObstaclesBonus;

		obstacleSlowImg = Util.loadBitmapFromAssets("game_obstacle_slow.png");
		obstacleBonusImg = Util.loadBitmapFromAssets("game_obstacle_bonus2.png");

		Block.setTextureLeft(Util.loadBitmapFromAssets("game_block_left1.png"));
		Block.setTextureMiddle(Util.loadBitmapFromAssets("game_block_middle1.png"));
		Block.setTextureRight(Util.loadBitmapFromAssets("game_block_right1.png"));
	...
	private void appendBlockToEnd(int BlockNumber)
	{
		...
		if(BlockNumber==-1){
			...
			if(distance <= Playerb.width)
				distance = Playerb.width+10;
		}else{
			distance = Playerb.width+5;
			switch (BlockNumber){
				case 1:
					newHeight=oldHeight;
...
//		newLeft = lastBlock.BlockRect.right + distance;
	...
	private void initializeObstacles(Boolean firstTime)
	{
		for(int i = 0; i < maxObstaclesJumper; i++)
		{
			if (firstTime)
			{
				obstacleDataJumper[i] = new ObstacleJumpb(-1000, 0, 0.9f, obstacleJumperWidth, obstacleJumperHeight, 'j', 60, 4);
			}
		...
		for(int i = 0; i < maxObstaclesBonus; i++)
		{
			if (firstTime)
			{
				obstacleDataBonus[i] = new ObstacleBonusb(-1000, 0, 0.9f, obstacleBonusWidth, obstacleBonusHeight, 'b');
	...
	private void appendObstaclesToEnd(Boolean spawnJumper, Boolean spawnSlower, Boolean spawnBonus)
	{
		...
		if (spawnJumper)
		{
			...
			float obstacleLeft;
			ObstacleJumpb newJumpObstacle = obstacleDataJumper[leftJumperIndex];
		...
		if (spawnBonus)
		{
			...
			ObstacleBonusb newBonus = obstacleDataBonus[leftBonusIndex];
