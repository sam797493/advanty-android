...
import games.adventure.com.advanty.ObstacleBonusc;
import games.adventure.com.advanty.ObstacleJumpc;
import games.adventure.com.advanty.OpenGLRenderer;
import games.adventure.com.advanty.Playerc;
...
public class Levelc {
	...
	public Levelc(Context context, OpenGLRenderer glrenderer, int _width, int _heigth) {
		if(Settings.RHDEBUG)
		...
		obstacleDataJumper = new ObstacleJumpc[maxObstaclesJumper];
		leftJumperIndex = 0;
		rightJumperIndex = maxObstaclesJumper;

		obstacleDataBonus = new ObstacleBonusc[maxObstaclesBonus];
		leftBonusIndex = 0;
		rightBonusIndex = maxObstaclesBonus;

		obstacleSlowImg = Util.loadBitmapFromAssets("game_obstacle_slow.png");
		obstacleBonusImg = Util.loadBitmapFromAssets("game_obstacle_bonus3.png");

		Block.setTextureLeft(Util.loadBitmapFromAssets("game_block_left3.png"));
		Block.setTextureMiddle(Util.loadBitmapFromAssets("game_block_middle3.png"));
		Block.setTextureRight(Util.loadBitmapFromAssets("game_block_right3.png"));
	...
	private void appendBlockToEnd(int BlockNumber)
	{
		...
		if(BlockNumber==-1){
			...
			if(distance <= Playerc.width)
				distance = Playerc.width+10;
		}else{
			distance = Playerc.width+5;
			switch (BlockNumber){
				case 1:
					newHeight=oldHeight;
...
//		newLeft = lastBlock.BlockRect.right + distance;
	...
	private void initializeObstacles(Boolean firstTime)
	{
		for(int i = 0; i < maxObstaclesJumper; i++)
		{
			if (firstTime)
			{
				obstacleDataJumper[i] = new ObstacleJumpc(-1000, 0, 0.9f, obstacleJumperWidth, obstacleJumperHeight, 'j', 60, 4);
			}
		...
		for(int i = 0; i < maxObstaclesBonus; i++)
		{
			if (firstTime)
			{
				obstacleDataBonus[i] = new ObstacleBonusc(-1000, 0, 0.9f, obstacleBonusWidth, obstacleBonusHeight, 'b');
	...
	private void appendObstaclesToEnd(Boolean spawnJumper, Boolean spawnSlower, Boolean spawnBonus)
	{
		...
		if (spawnJumper)
		{
			...
			float obstacleLeft;
			ObstacleJumpc newJumpObstacle = obstacleDataJumper[leftJumperIndex];
		...
		if (spawnBonus)
		{
			...
			ObstacleBonusc newBonus = obstacleDataBonus[leftBonusIndex];
