...
public class Playerd {
	...
	public Playerd(Context context, OpenGLRenderer glrenderer, int ScreenHeight) {
		...
		playerSpriteImg = Util.loadBitmapFromAssets("game_character_spritesheet4.png");
	...
	public boolean update() {
		...
		for (int i = 0; i < Leveld.maxBlocks; i++)
		{
			if( checkIntersect(playerRect, Leveld.blockData[i].BlockRect) )
			{
				if(lastPosY >= Leveld.blockData[i].mHeight && velocity <= 0)
				{
					y=Leveld.blockData[i].mHeight;
					velocity = 0;
					reachedPeak = false;
					jumping = false;
					onGround = true;
	...
	public boolean collidedWithObstacle(float LeveldPosition) {
		
		for(int i = 0; i < Leveld.maxObstaclesJumper; i++)
		{
			ObstacleRect.left =  (int)Leveld.obstacleDataJumper[i].x;
			ObstacleRect.top = (int)Leveld.obstacleDataJumper[i].y+(int)Leveld.obstacleDataJumper[i].height; 
			ObstacleRect.right = (int)Leveld.obstacleDataJumper[i].x+(int)Leveld.obstacleDataJumper[i].width;
			ObstacleRect.bottom = (int)Leveld.obstacleDataJumper[i].y;
			
			if( checkIntersect(playerRect, ObstacleRect) && !Leveld.obstacleDataJumper[i].didTrigger)
			{
				Leveld.obstacleDataJumper[i].didTrigger=true;
		...
		for(int i = 0; i < Leveld.maxObstaclesSlower; i++)
		{
			ObstacleRect.left =  (int)Leveld.obstacleDataSlower[i].x;
			ObstacleRect.top = (int)Leveld.obstacleDataSlower[i].y+(int)Leveld.obstacleDataSlower[i].height; 
			ObstacleRect.right = (int)Leveld.obstacleDataSlower[i].x+(int)Leveld.obstacleDataSlower[i].width;
			ObstacleRect.bottom = (int)Leveld.obstacleDataSlower[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Leveld.obstacleDataSlower[i].didTrigger)
			{
				Leveld.obstacleDataSlower[i].didTrigger=true;
		...
		for(int i = 0; i < Leveld.maxObstaclesBonus; i++)
		{
			ObstacleRect.left =  (int)Leveld.obstacleDataBonus[i].x;
			ObstacleRect.top = (int)Leveld.obstacleDataBonus[i].y+(int)Leveld.obstacleDataBonus[i].height; 
			ObstacleRect.right = (int)Leveld.obstacleDataBonus[i].x+(int)Leveld.obstacleDataBonus[i].width;
			ObstacleRect.bottom = (int)Leveld.obstacleDataBonus[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Leveld.obstacleDataBonus[i].didTrigger)
			{
				SoundManager.playSound(8, 1);
				Leveld.obstacleDataBonus[i].didTrigger=true;

				bonusItems++;
				Leveld.obstacleDataBonus[i].z= -1;
			}
		}
