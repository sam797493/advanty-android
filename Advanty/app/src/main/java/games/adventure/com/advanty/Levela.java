...
import games.adventure.com.advanty.ObstacleBonusa;
import games.adventure.com.advanty.ObstacleJumpa;
import games.adventure.com.advanty.Playera;
import games.adventure.com.advanty.RHDrawable;
...
public class Levela {
	...
	private games.adventure.com.advanty.OpenGLRenderer renderer;
	...
	public Levela(Context context, games.adventure.com.advanty.OpenGLRenderer glrenderer, int _width, int _heigth) {
		if(Settings.RHDEBUG)
			Log.d("debug", "in Levelc constructor");
		...
		baseSpeedStart = games.adventure.com.advanty.Util.getPercentOfScreenWidth(0.095f);
		baseSpeed = baseSpeedStart;		
		baseSpeedMaxStart = games.adventure.com.advanty.Util.getPercentOfScreenWidth(0.2f);
		baseSpeedMax = baseSpeedMaxStart;
		baseSpeedAcceleration = baseSpeed*0.025f;
		
		extraSpeedStart = games.adventure.com.advanty.Util.getPercentOfScreenWidth(0.025f);
		extraSpeed = extraSpeedStart;
		extraSpeedMaxStart = games.adventure.com.advanty.Util.getPercentOfScreenWidth(0.5f);
		...
		obstacleJumperWidth = games.adventure.com.advanty.Util.getPercentOfScreenWidth(4.875f);
		obstacleJumperHeight = games.adventure.com.advanty.Util.getPercentOfScreenHeight(14.25f);
		
		obstacleSlowerWidth = games.adventure.com.advanty.Util.getPercentOfScreenWidth(6);
		obstacleSlowerHeight= games.adventure.com.advanty.Util.getPercentOfScreenHeight(6);
		
		obstacleBonusWidth = games.adventure.com.advanty.Util.getPercentOfScreenWidth(5);
		obstacleBonusHeight = obstacleBonusWidth* games.adventure.com.advanty.Util.mWidthHeightRatio;
		obstacleBonusDistanceToBlock = games.adventure.com.advanty.Util.getPercentOfScreenHeight(12);
		...
		obstacleDataSlower = new games.adventure.com.advanty.Obstacle[maxObstaclesSlower];
		leftSlowerIndex = 0;
		rightSlowerIndex = maxObstaclesSlower;
		
		obstacleDataJumper = new ObstacleJumpa[maxObstaclesJumper];
		leftJumperIndex = 0;
		rightJumperIndex = maxObstaclesJumper;
		
		obstacleDataBonus = new ObstacleBonusa[maxObstaclesBonus];
		leftBonusIndex = 0;
		rightBonusIndex = maxObstaclesBonus;

		obstacleSlowImg = games.adventure.com.advanty.Util.loadBitmapFromAssets("game_obstacle_slow.png");
		obstacleBonusImg = games.adventure.com.advanty.Util.loadBitmapFromAssets("game_obstacle_bonus.png");
		
		Block.setTextureLeft(games.adventure.com.advanty.Util.loadBitmapFromAssets("game_block_left1.png"));
		Block.setTextureMiddle(games.adventure.com.advanty.Util.loadBitmapFromAssets("game_block_middle1.png"));
		Block.setTextureRight(games.adventure.com.advanty.Util.loadBitmapFromAssets("game_block_right1.png"));
		...
		mWaves.loadBitmap(games.adventure.com.advanty.Util.loadBitmapFromAssets("game_waves.png")
	...
	public void update() {
		...
		synchronized (blockData) {
			...
			if(games.adventure.com.advanty.Util.getTimeSinceRoundStartMillis() > timeUntilNextSpeedIncreaseMillis){
				timeUntilNextSpeedIncreaseMillis += Settings.timeToFurtherSpeedIncreaseMillis;
				baseSpeedMax += games.adventure.com.advanty.Util.getPercentOfScreenWidth(0.075f);
			}
	...
	private void appendBlockToEnd(int BlockNumber)
	{
		...
		if(BlockNumber==-1){
			...
			if(games.adventure.com.advanty.Util.getTimeSinceRoundStartMillis() > Settings.timeUntilLongBlocksStopMillis){
			...
			if(distance <= Playera.width)
				distance = Playera.width+10;
		}else{	
			distance = Playera.width+5;
			switch (BlockNumber){
				case 1:
					newHeight=oldHeight; 
					newWidth= games.adventure.com.advanty.Util.getPercentOfScreenWidth(80);
					break;
				case 2:
					newHeight=oldHeight+ games.adventure.com.advanty.Util.getPercentOfScreenHeight(8);
					newWidth= games.adventure.com.advanty.Util.getPercentOfScreenWidth(80);
					break;
				case 3:
					newHeight=oldHeight;
					...
					break;
				case 4:
					newHeight=oldHeight+ games.adventure.com.advanty.Util.getPercentOfScreenHeight(9);
...
//		newLeft = lastBlock.BlockRect.right + distance; 
	...
	private void initializeObstacles(Boolean firstTime)
	{
		for(int i = 0; i < maxObstaclesJumper; i++)
		{
			if (firstTime)
			{
				obstacleDataJumper[i] = new ObstacleJumpa(-1000, 0, 0.9f, obstacleJumperWidth, obstacleJumperHeight, 'j', 60, 4);
			}
		...
		for(int i = 0; i < maxObstaclesSlower; i++)
		{
			if (firstTime)
			{
				obstacleDataSlower[i] = new games.adventure.com.advanty.Obstacle(-1000, 0, 0.9f, obstacleSlowerWidth, obstacleSlowerHeight, 's');
		...
		for(int i = 0; i < maxObstaclesBonus; i++)
		{
			if (firstTime)
			{
				obstacleDataBonus[i] = new ObstacleBonusa(-1000, 0, 0.9f, obstacleBonusWidth, obstacleBonusHeight, 'b');
				renderer.addMesh(obstacleDataBonus[i]);
				obstacleDataBonus[i].loadBitmap(
	...
	private void appendObstaclesToEnd(Boolean spawnJumper, Boolean spawnSlower, Boolean spawnBonus)
	{
		...
		if (spawnJumper)
		{
			...
			float obstacleLeft;
			ObstacleJumpa newJumpObstacle = obstacleDataJumper[leftJumperIndex];
		...
		if (spawnBonus)
		{
			...
			ObstacleBonusa newBonus = obstacleDataBonus[leftBonusIndex];
