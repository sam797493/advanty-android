...
public class Playerb {
	...
	public Playerb(Context context, OpenGLRenderer glrenderer, int ScreenHeight) {
		...
		playerSpriteImg = Util.loadBitmapFromAssets("game_character_spritesheet2.png");
	...
	public boolean update() {
		...
		for (int i = 0; i < Levelb.maxBlocks; i++)
		{
			if( checkIntersect(playerRect, Levelb.blockData[i].BlockRect) )
			{
				if(lastPosY >= Levelb.blockData[i].mHeight && velocity <= 0)
				{
					y=Levelb.blockData[i].mHeight;
					velocity = 0;
					reachedPeak = false;
					jumping = false;
					onGround = true;
	...
	public boolean collidedWithObstacle(float LevelbPosition) {

		for(int i = 0; i < Levelb.maxObstaclesJumper; i++)
		{
			ObstacleRect.left =  (int)Levelb.obstacleDataJumper[i].x;
			ObstacleRect.top = (int)Levelb.obstacleDataJumper[i].y+(int)Levelb.obstacleDataJumper[i].height;
			ObstacleRect.right = (int)Levelb.obstacleDataJumper[i].x+(int)Levelb.obstacleDataJumper[i].width;
			ObstacleRect.bottom = (int)Levelb.obstacleDataJumper[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levelb.obstacleDataJumper[i].didTrigger)
			{
				Levelb.obstacleDataJumper[i].didTrigger=true;
		...
		for(int i = 0; i < Levelb.maxObstaclesSlower; i++)
		{
			ObstacleRect.left =  (int)Levelb.obstacleDataSlower[i].x;
			ObstacleRect.top = (int)Levelb.obstacleDataSlower[i].y+(int)Levelb.obstacleDataSlower[i].height;
			ObstacleRect.right = (int)Levelb.obstacleDataSlower[i].x+(int)Levelb.obstacleDataSlower[i].width;
			ObstacleRect.bottom = (int)Levelb.obstacleDataSlower[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levelb.obstacleDataSlower[i].didTrigger)
			{
				Levelb.obstacleDataSlower[i].didTrigger=true;
		...
		for(int i = 0; i < Levelb.maxObstaclesBonus; i++)
		{
			ObstacleRect.left =  (int)Levelb.obstacleDataBonus[i].x;
			ObstacleRect.top = (int)Levelb.obstacleDataBonus[i].y+(int)Levelb.obstacleDataBonus[i].height;
			ObstacleRect.right = (int)Levelb.obstacleDataBonus[i].x+(int)Levelb.obstacleDataBonus[i].width;
			ObstacleRect.bottom = (int)Levelb.obstacleDataBonus[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levelb.obstacleDataBonus[i].didTrigger)
			{
				SoundManager.playSound(8, 1);
				Levelb.obstacleDataBonus[i].didTrigger=true;

				bonusItems++;
				Levelb.obstacleDataBonus[i].z= -1;
			}
		}
