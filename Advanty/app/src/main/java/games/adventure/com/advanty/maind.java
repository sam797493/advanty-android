...
public class maind extends Activity {
	...
	public class RunnersHighView extends GLSurfaceView implements Runnable {
		private Playerd player = null;
		private Leveld level;
...
//			initialize();
		...
		private void initialize() {
			...
			try {
				...
				background.loadLayerFar(Util.loadBitmapFromAssets("game_background_layer_4.png"));
				sleep();
				...
				try {
					...
					background.loadLayerFar(
							Util.loadBitmapFromAssets("game_background_layer_4.png"));
					sleep();
			...
			try {
				background.loadLayerMiddle(Util.loadBitmapFromAssets("bg2.png"));
				sleep();
			} catch (OutOfMemoryError oome) {
				System.gc();
				try {
			...
			try {
				background.loadLayerNear(Util.loadBitmapFromAssets("bg2.png"));
				sleep();
			...
			resetButton = new Button(
					...
					Util.getPercentOfScreenHeight(17));
			...
			player = new Playerd(getApplicationContext(), mRenderer, height);
			sleep();

			level = new Leveld(context, mRenderer, width, height);
			sleep();
...
//		    loadingDialog.setMessage("Loading Highscore ...");
			...
			CounterYourScoreDrawable = new RHDrawable(
					...
					height-Util.getPercentOfScreenHeight(19),
					0.9f,
					Util.getPercentOfScreenWidth(28),
					Util.getPercentOfScreenHeight(19));
