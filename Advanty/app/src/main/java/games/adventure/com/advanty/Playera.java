...
public class Playera{
	...
	public Playera(Context context, OpenGLRenderer glrenderer, int ScreenHeight) {
	...
	public boolean update() {
		...
		for (int i = 0; i < Levela.maxBlocks; i++)
		{
			if( checkIntersect(playerRect, Levela.blockData[i].BlockRect) )
			{
				if(lastPosY >= Levela.blockData[i].mHeight && velocity <= 0)
				{
					y=Levela.blockData[i].mHeight;
					velocity = 0;
					reachedPeak = false;
					jumping = false;
					onGround = true;
	...
	public boolean collidedWithObstacle(float LevelaPosition) {

		for(int i = 0; i < Levela.maxObstaclesJumper; i++)
		{
			ObstacleRect.left =  (int)Levela.obstacleDataJumper[i].x;
			ObstacleRect.top = (int)Levela.obstacleDataJumper[i].y+(int)Levela.obstacleDataJumper[i].height;
			ObstacleRect.right = (int)Levela.obstacleDataJumper[i].x+(int)Levela.obstacleDataJumper[i].width;
			ObstacleRect.bottom = (int)Levela.obstacleDataJumper[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levela.obstacleDataJumper[i].didTrigger)
			{
				Levela.obstacleDataJumper[i].didTrigger=true;
		...
		for(int i = 0; i < Levela.maxObstaclesSlower; i++)
		{
			ObstacleRect.left =  (int)Levela.obstacleDataSlower[i].x;
			ObstacleRect.top = (int)Levela.obstacleDataSlower[i].y+(int)Levela.obstacleDataSlower[i].height;
			ObstacleRect.right = (int)Levela.obstacleDataSlower[i].x+(int)Levela.obstacleDataSlower[i].width;
			ObstacleRect.bottom = (int)Levela.obstacleDataSlower[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levela.obstacleDataSlower[i].didTrigger)
			{
				Levela.obstacleDataSlower[i].didTrigger=true;
		...
		for(int i = 0; i < Levela.maxObstaclesBonus; i++)
		{
			ObstacleRect.left =  (int)Levela.obstacleDataBonus[i].x;
			ObstacleRect.top = (int)Levela.obstacleDataBonus[i].y+(int)Levela.obstacleDataBonus[i].height;
			ObstacleRect.right = (int)Levela.obstacleDataBonus[i].x+(int)Levela.obstacleDataBonus[i].width;
			ObstacleRect.bottom = (int)Levela.obstacleDataBonus[i].y;

			if( checkIntersect(playerRect, ObstacleRect) && !Levela.obstacleDataBonus[i].didTrigger)
			{
				SoundManager.playSound(8, 1);
				Levela.obstacleDataBonus[i].didTrigger=true;

				bonusItems++;
				Levela.obstacleDataBonus[i].z= -1;
			}
		}
